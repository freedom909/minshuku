// Test becomeHost mutation for existing user mpeg56@gmail.com

async function findUserByEmail(email) {
  try {
    console.log(`🔍 Searching for existing user: ${email}`);
    
    // Since we can't query users directly due to resolver issues,
    // let's try to sign in first to get the user ID
    ...
    if (response.ok) {
      ...
      if (result.errors) {
        ...
        // If password is wrong, try with empty password or different approach
        if (result.errors[0].message.includes('password')) {
          console.log('💡 Password might be incorrect. Trying alternative approach...');
          return await tryAlternativeApproach(email);
        }
        return null;
      }
...
async function tryAlternativeApproach(email) {
  try {
    console.log('🔧 Trying alternative approach to find user ID...');
    ...
    if (response.ok) {
      const result = await response.json();
      console.log('Alternative signIn result:', JSON.stringify(result, null, 2));
      
      if (result.data && result.data.signIn && result.data.signIn.userId) {
        return result.data.signIn.userId;
      }
    }
...
async function main() {
  ...
  console.log('🚀 Testing becomeHost for existing user...');
  console.log('==========================================');
  ...
  if (userId) {
    ...
    console.log('1. Check the exact password for mpeg56@gmail.com');
    console.log('2. Use a different test user email');
    console.log('3. Check MongoDB directly for user IDs');
  }
}
